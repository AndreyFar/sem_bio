...
def show(df_loaded):
    ...
    # Vývoj počtu diagnóz v čase podľa skupín
    ...
    if not diag_over_time.empty:
        diag_over_time_pivot = diag_over_time.pivot(index='rok_vysetrenia', columns='skupina_diagnozy',
                                                    values='pocet').fillna(0)

        st.subheader("Tabuľkový prehľad počtu diagnóz podľa skupín a rokov")
        st.dataframe(diag_over_time_pivot.style.format("{:.0f}"))

        # Grafický prehľad vývoja počtu diagnóz
        ...
        if selected_groups_for_plot_input:
            ...
            if not df_to_plot_non_zero.empty and not df_to_plot_non_zero.columns.empty:
                ...
                st.info(
                    "Pre vybrané skupiny neboli nájdené žiadne nenulové dáta na zobrazenie v grafe, alebo všetky vybrané skupiny mali nulové počty.")
